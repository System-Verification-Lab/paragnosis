...
MultiGraphProbability::MultiGraphProbability(){
    manager_ = NULL;
    bn_ = NULL;
    g_ = NULL;
    terminal_ = NULL;
...
void MultiGraphProbability::SetManager(bnc::manager_t *manager){
    manager_ = manager;
...
void MultiGraphProbability::RuntimeInit(const partition_t &kPartition){
    const unsigned int kNrBnVariables = bn_->get_nr_variables();

    cpt_ctr_.resize(kNrBnVariables);
    cpt_probabilities_.resize(kNrBnVariables);
    for(auto it = kPartition.set.begin(); it != kPartition.set.end(); it++){
        BayesNode *n = g_->get_node(*it);
        cpt_probabilities_[*it] = &(n->GetProbabilities()[0]);
        cpt_ctr_[*it].SetDimension(n->GetDimensions());
    }
}

template <bool STRUCTURE>
void MultiGraphProbability::Compile(){
    assert(initialized_);
    Compile<STRUCTURE>(spanningtree_);
}

namespace bncp {
inline bool Expand(const SpanningTreeNode *kSpanningNode, const Variable kVariable, bool *expands){
...
using namespace bncp;

void MultiGraphProbability::AddProbabilities(MultiGraphProbability::Node *node, const SpanningTree *kSpanningTree, const SpanningTreeNode *kSpanningNode, const XAry &xary, const unsigned int kDimension){
    const auto &kMessages = kSpanningNode->messages;

    Edge *edge = node->EdgeBegin();
    const auto kNrProbabilities = kMessages.size();
    for(unsigned int value = 0; value < kDimension; value++){
        edge->probability = 1;
        for(unsigned int i = 0; i < kNrProbabilities; i++){
            ...
            const unsigned int kProbabilityIndex = ctr.GetDecimal();
            edge->probability *= cpt_probabilities_[kCpt][kProbabilityIndex];
            if(edge->probability  == 0){ // determinism
                if(OPT_DETERMINISM)
                    edge->to = terminal_;
                break;
            }
...
void MultiGraphProbability::AddEdges(MultiGraphProbability::Node **to, const XAry &xary, XAry &xary_child, const SpanningTreeNode *kChildSpanningNode, const unsigned int kDimension, const bool kExpand){
    #ifdef DEBUG
...
void MultiGraphProbability::AddEdges(MultiGraphProbability::Node *node, const XAry &xary, XAry &xary_child, const SpanningTreeNode *kChildSpanningNode, const unsigned int kDimension, const bool kExpand){
    #ifdef DEBUG
    assert((xary_child.Max(kChildSpanningNode->xary.restrict_dimension) == 1 || xary_child.Max(kChildSpanningNode->xary.restrict_dimension) == kDimension) && "Parent has too many children");
    #endif
...
template<bool STRUCTURE>
void MultiGraphProbability::CreateOrNodes(const SpanningTree *kSpanningTree, const SpanningTreeNode *kSpanningNode){
    ...
    do {
        ...
        AddProbabilities(node, kSpanningTree, kSpanningNode, xary, kDimension);
...
template<bool STRUCTURE>
void MultiGraphProbability::CreateAndNodes(const SpanningTree *kSpanningTree, const SpanningTreeNode *kSpanningNode){
    // prepare counters
    XAry xary;
    xary.SetDimension(bn_,kSpanningNode->spanning);
    xary.SetZero();
...
template<bool STRUCTURE>
void MultiGraphProbability::CreateNodes(const SpanningTree *kSpanningTree, const SpanningTreeNode *kSpanningNode){
...
template <bool STRUCTURE>
MultiGraphProbability::Node* MultiGraphProbability::Canonical(MultiGraphProbability::Node *node){
    if(STRUCTURE){
        //#ifdef DEBUG
...
template<bool STRUCTURE>
MultiGraphProbability::Node* MultiGraphProbability::Compile(const SpanningTree *kSpanningTree, const SpanningTreeNode *kSpanningRoot, bool traversed[]){
...
template <bool STRUCTURE>
void MultiGraphProbability::Compile(const SpanningTree &kSpanningTree){
...
void MultiGraphProbability::DumpDot(std::vector<MultiGraphProbability>& graphs){
    if(graphs.size() == 1){
        graphs[0].DumpDot();
    } else {
...
void MultiGraphProbability::DumpDot(const int kPartitionId){
...
void MultiGraphProbability::DumpDotOrdering(FILE *file){
...
void MultiGraphProbability::DumpDotNodes(FILE *file, std::map< Variable, std::vector<const MultiGraphProbability::Node*>>& and_nodes, std::vector< std::vector<const MultiGraphProbability::Node*> >& nodes_per_level, std::vector< std::set< Variable > >& variables_per_level){
...
void MultiGraphProbability::DumpDotEdges(FILE *file, std::map< Variable, std::vector<const MultiGraphProbability::Node*>>& and_nodes, std::vector< std::vector<const MultiGraphProbability::Node*> >& nodes_per_level){
    // dump AND edges
    fprintf(file, "\n    // ==================== EDGES ===================\n\n");
    ...
    for(unsigned int level = 0; level < nodes_per_level.size(); level++){
        ...
        for(auto it = kNodes.begin(); it != kNodes.end(); it++){
            ...
            for(unsigned int value = 0; value < kNode->size; value++){
                std::string prob_str;
                auto probability = kNode->edges[value].probability;
                if(probability == 0)
                    prob_str = "0";
                else if(probability == 1)
                    prob_str = "1";
                else if(probability < 0.001)
                    prob_str = stringf("> 0.001");
                else prob_str = stringf("%.3lf",kNode->edges[value].probability);
                std::string value_name = bn_->get_node_value_name(kNode->variable,value);
                fprintf(file, "    %lu -> %lu [fontsize=8,label=\"%s\\n%s\"];\n",(size_t) kNode, (size_t) kNode->edges[value].to,value_name.c_str(),prob_str.c_str());
            }
        }
    }
...
void MultiGraphProbability::DumpDotPrepare(std::map< Variable, std::vector<const MultiGraphProbability::Node*>>& and_nodes, std::vector< std::vector<const MultiGraphProbability::Node*> >& nodes_per_level, std::vector< std::set< Variable > >& variables_per_level){

    std::queue<const MultiGraphProbability::Node*> s;
    s.push(GetRoot());
    size_t level = 0;
    bool add_delim = true;
...
void MultiGraphProbability::DumpDot(std::string filename){
    std::map< Variable, std::vector<const MultiGraphProbability::Node*>> and_nodes;
    std::vector< std::vector<const MultiGraphProbability::Node*> > nodes_per_level;
...
void MultiGraphProbability::DumpDD(std::vector<MultiGraphProbability> &graphs){
    if(graphs.size() == 1){
        graphs[0].DumpDD();
    } else {
        for(unsigned int i = 0; i < graphs.size(); i++)
...
void MultiGraphProbability::DumpDD(const int kPartitionId){
    auto& files = manager_->get_files();
    ...
    if(OPT_WRITE_BINARY)
        DumpDD<true>(files.get_filename_c(AC,aux));
    else DumpDD<false>(files.get_filename_c(AC,aux));
}

template <bool kBinary>
void MultiGraphProbability::DumpDD(std::string filename){
    FILE *file;
    if(kBinary)
    ...
    if(file){
        ...
        index[terminal_->GetId()] = 0;   // terminal

        std::unordered_set<size_t> done;
        done.insert(terminal_->GetId());

        const auto *kRoot = GetRoot();
        std::stack<const MultiGraphProbability::Node*> s;
        s.push(kRoot);
        unsigned int index_counter = 1;
        while(!s.empty()){
            const MultiGraphProbability::Node *kNode = s.top();
            ...
            if(done.insert(kNodeId).second){
                ...
                index[kNodeId] = index_counter++;
            }
        }

        // dump nodes
        Size size = GetSize();
        const bool kIsTree = spanningtree_.IsTree();
        if(kBinary){
            fwrite(&kIsTree, sizeof(bool), 1, file);
            fwrite(&size.nodes, sizeof(decltype(size.nodes)), 1, file);
            fwrite(&size.edges, sizeof(decltype(size.edges)), 1, file);
        } else {
            fprintf(file, "p-wpbdd");
            if(kIsTree)
                fprintf(file, " tree");
        ...
        while(!s.empty()){
            ...
            if(done.insert(kNodeId).second){
                #ifdef DEBUG
                const auto kNodeIndex = index[kNodeId];
                if(!(lines == kNodeIndex))
                    fprintf(stderr, "lines: %lu - Nodes: %lu\n", lines, kNodeIndex);
                assert(lines == kNodeIndex);
                #endif
                ...
                // dump node variable
                if(kBinary)
                    fwrite(&(kNode->variable), sizeof(decltype(kNode->variable)), 1, file);
                else fprintf(file, " %lu", kNode->GetVariable());         // variable

                // dump node nr of edges
                if(kBinary)
                    fwrite(&(kNode->size), sizeof(decltype(kNode->size)), 1, file);
                else fprintf(file, " %lu",kNode->size);

                // dump edges
                for(unsigned int i = 0; i < kNode->size; i++){
                    const size_t kIndex = index[kNode->edges[i].to->GetId()];
                    if(kBinary){
                        fwrite(&kIndex, sizeof(decltype(kIndex)), 1, file);
                        if(!kIsAnd)
                            fwrite(&(kNode->edges[i].probability), sizeof(decltype(kNode->edges[i].probability)), 1, file);
                    } else {
                        fprintf(file, " %lu", kIndex);
                        if(!kIsAnd)
                            fprintf(file, " %lf", kNode->edges[i].probability);
                    }
                }
                if(!kBinary)
                    fprintf(file,"\n");
        ...
        fclose(file);
    } else compiler_write_exception("Could not open p-WPBDD file '%s'", filename.c_str());
}


MultiGraphProbability::Size MultiGraphProbability::GetSize() const {
    ...
    while(!s.empty()){
        ...
        if(!done[kNodeId]){
            ...
            if(kNode->IsAnd()){
                ...
                size.or_nodes++;
                size.probabilities += kNode->size;
                size.operators += kNode->size * 2  + kNode->size;
            }
        }
    }
...
bool MultiGraphProbability::HasSpanningTree() const {
    return !spanningtree_.Empty();
}

const SpanningTree& MultiGraphProbability::GetSpanningTree() const {
    return spanningtree_;
}

void MultiGraphProbability::InitSpanningChain(bnc::manager_t* m, const bn_partition_t &kBnPartition){
    SetManager(m);
    assert(!initialized_);
...
void MultiGraphProbability::AllocateCacheLayer(const SpanningTreeNode* const kSpanningNode, const bool kHasMap){
    assert(kSpanningNode->index < cache_.size());

    auto &local_cache = cache_[kSpanningNode->index];
    local_cache.SetNodeProperties(kSpanningNode->GetNrEdgesPerOr(), kSpanningNode->GetNrEdgesPerAnd());
    local_cache.Resize(
...
void MultiGraphProbability::AllocateCache(const bool kHasMap){
...
void MultiGraphProbability::InitSpanningTree(manager* manager, const bn_partition_t &kBnPartition){
    SetManager(manager);
    assert(!initialized_);
    const partition_t &kPartition = kBnPartition.partition;
...
const MultiGraphProbability::Node* MultiGraphProbability::GetRoot() const {
    return root_;
}

template void MultiGraphProbability::Compile<true>();
template void MultiGraphProbability::Compile<false>();
