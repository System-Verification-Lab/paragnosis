...
std::string ModelCounter<ModelType::PWPBDD>::Description<N>(){
    return "Bottom up traverals of partitions, using pre-initialized evidence cache and cached runtime arrays. Partitions are traversed per tier, starting at the bottom tier.";
}
...
void ModelCounter<ModelType::PWPBDD>::TraverseTiers<N>(
    ...
    for(int tier = kTiers-1; tier >= 0; tier--){
        ...
        for(auto it = kNodeIdList.begin(); it != kNodeIdList.end(); it++){
            ...
            if(tier < kTiers-1)
                probability = TraversePartition<N>(kArchitecture, cache, evidence_list, kConditionTierList, tier, kNodeId);
            else probability = Traverse<N>(kArchitecture, cache, evidence_list, kConditionTierList, tier, kNodeId);
        }
    }
}
...
probability_t ModelCounter<ModelType::PWPBDD>::Posterior<N>(){
    ...
    #ifdef DEBUG
    QueryProbabilities &probs = manager.probabilities["PWPBDD_5_"];
