...
std::string ModelCounter<ModelType::PWPBDD>::Description<N>(){
    return "Bottom up traverals of partitions. Storage is local. Partitions are traversed per tier, starting at the bottom tier.";
}
...
void ModelCounter<ModelType::PWPBDD>::TraverseTiers<N>(
    ...
    const unsigned int kBuffer = manager.buffer;
    ...
    for(int tier = kTiers-1; tier >= 0; tier--){
        ...
        for(unsigned int i = 0; i < kSpanningSet.size(); i++){
            ...
            const bool kIsConditioned = kConditionTierList[kVariable] == 0;
            restrict_dimension.push_back(kIsConditioned);
            xary[i] = (kIsConditioned?kEvidenceList[kVariable]:0);
        }

        do {
            // prepare evidence
            EvidenceList evidence_list = kEvidenceList;
            for(unsigned int i = 0; i < kSpanningSet.size(); i++)
                evidence_list[kSpanningSet[i]] = xary[i];

            if(kBuffer) usleep(kBuffer);
            // do traversal
...
probability_t ModelCounter<ModelType::PWPBDD>::Posterior<N>(){
    ...
    #ifdef DEBUG
    QueryProbabilities &probs = manager.probabilities["PWPBDD_3_"];
    probs.p = 1;
