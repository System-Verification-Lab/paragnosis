...
namespace bnmc_2_ {
    ...
    struct TaskQueues {
        TaskQueues(unsigned int size) : tasks_queue(size){};
        BlockingReaderWriterQueue< TaskItem > task_queue[MAX_THREADS];
        std::queue< TaskItem > task_queue_check[MAX_THREADS];
        BlockingConcurrentQueue<TasksItem> tasks_queue;
    };
...
void* pthread_worker(void *in){
    ...
    auto &queue = queues->task_queue[kThreadId];
    auto &queue2 = queues->task_queue_check[kThreadId];
    auto &scheduler_queue = queues->tasks_queue;
    while(true){
        TaskItem task;
        queue.wait_dequeue(task);
        TaskItem task2 = queue2.front();
        queue2.pop();
...
void pthread_scheduler(Timer *t){
    ...
    auto &task_queue = queues->task_queue;
    auto &task_queue2 = queues->task_queue_check;
    auto &queue = queues->tasks_queue;
        ...
        if(item.with_query_variable){
            if(item.task_group_id == kTasksFinished)
                ...
                for(auto it = kTaskGroup.nodes.begin(); it != kTaskGroup.nodes.end(); it++){
                    ...
                    task_queue[thread_id].enqueue({*it,kTaskGroup.tier,item.with_query_variable});
                    task_queue2[thread_id].push({*it,kTaskGroup.tier,item.with_query_variable});
...
probability_t ModelCounter<ModelType::PWPBDD>::ParallelPosterior<N>(const unsigned int kWorkers, Timer* t){
    ...
    // set initial tasks
    auto &scheduler_queue = queues->tasks_queue;
    scheduler_queue.enqueue({0,true});
    if(gHasQueryVariable)
        scheduler_queue.enqueue({0,false});
    ...
    #ifdef DEBUG
    QueryProbabilities &probs = manager.probabilities["PPWPBDD_2_"];
