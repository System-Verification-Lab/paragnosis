...
class Bdd:
    ...
    def create_partitioning(this):
        ...
        if this.overwrite or not os.path.exists(this.part):
            cmd = "{:s} {:s} -t wpbdd -o no_compile=1 -w elim={:s} -o partitions={:d} -w part={:s}".format(this.compiler,this.hugin,this.num,this.partitions,this.part)
            misc.call(cmd,this.verbose)
        else:
            term.write("    [SKIPPED]  \n")
    ...
    def create_composition_ordering(this,):
        ...
        if this.overwrite or not os.path.exists(this.comp):
            cmd = "{:s} {:s} -t wpbdd -o no_compile=1 -r elim={:s} -r part={:s} -w comp={:s}".format(this.compiler,this.hugin,this.num,this.part,this.comp)
            misc.call(cmd,this.verbose)
        else:
            term.write("    [SKIPPED]  \n")
